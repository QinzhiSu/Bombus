...
#列出合并基因下的文件
path <- '/home/huyunan/bombus/02metagenome/midas/result/genes_merge'
setwd(path)
ls_dir <- list.dirs(path = path, full.names = TRUE, recursive = FALSE)

#读入dbcan注释文件
dbcan_file <- '/home/muxiaohuan/database/MXH_bee_gut_midas_database/bee_gut_pangenomes/pangenomes_genes_drep.faa.dbcan'
dbcan_index <- read.delim(file = dbcan_file, header = TRUE, sep = '\t')

#读入样本文件
sample_file <- '/home/huyunan/bombus/02metagenome/midas/data/sample.txt'
